    ...
    public void update(Session session) {
        try {
            PreparedStatement updateData = connection.prepareStatement(
                    "UPDATE session SET session_start = ?, " +
                            "session_end = ?, session_length = ? WHERE session_id = ? AND user_id = ?"
            );
            ...
            updateData.setInt(4, session.getSessionID());
            updateData.setInt(5, session.getUserID());
            updateData.execute();
